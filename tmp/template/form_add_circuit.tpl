...
<form class="animated zoomInRight" id="AddFormCircuit">
    ...
    <div>
        <label for="exampleInputEmail1" id="label_description_circuit">{$voc["lb_description"]}</label>
    ...
    <div class="form-group">
        ...
        <select class="form-control"  id="SelectTheme" name="SelectTheme" style="width:30%;display: inline;">
            {include file="select_themes.tpl" arr_theme_circuit=$voc["arr_theme_circuit"]}
        </select>
        <input type="button" value="{$voc["btn_add_theme"]}" class="btn btn-primary" style="display: inline" onclick="BtnViewNewTheme()">
    </div>
    ...
    <div class="form-group animated zoomInRight" style="width: 35%">
        <label>{$voc["lb_ville_depart"]}</label>
