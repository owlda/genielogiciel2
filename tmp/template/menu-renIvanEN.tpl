<nav class="navbar navbar-light navbar-expand-md navigation-clean-button bg-warning">
    <div class="container"><img src="../assets/img/travel-icon.png" style="width: 30px;height: 30px;margin-right: 5px;"><a class="navbar-brand" href="#">Voyages Merveilleux</a><button data-toggle="collapse" class="navbar-toggler" data-target="#navcol-1"><span class="sr-only">Toggle navigation</span><span class="navbar-toggler-icon"></span></button>
        <div class="collapse navbar-collapse"
             id="navcol-1">
            <ul class="nav navbar-nav mr-auto">
                <li class="nav-item" role="presentation"><a class="nav-link" href="http://localhost/files/Web3/circuit.in/www/pages/admin.php">Travels</a></li>
                <li class="nav-item" role="presentation"><a class="nav-link" href="http://localhost/files/Web3/circuit.in/www/pages/stat.php">EN_Stat</a></li>
                <li class="nav-item" role="presentation"><a class="nav-link" href="http://localhost/files/Web3/circuit.in/www//uploadimages.php">EN_Images</a></li>
            </ul>
            <span class="navbar-text actions"> <a class="login" href="#">Admin</a>

            </span>
            <select id="lang" style="margin: 5px;" onchange="ChangeCokies();">
                <option value="" selected>FR</option>
                <option value="en">EN</option>
            </select>
        </div>
    </div>
</nav>
